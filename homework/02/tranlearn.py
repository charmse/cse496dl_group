...
flags.DEFINE_string('save_dir', '/work/cse496dl/vsunkara/cse496dl_group/homework/02/models/', 'directory where model graph and weights are saved')
...
def main(argv):
    ...
    myfile = open(save_dir + 'output/' + save_prefix + 'model_'  + str(learning_rate) + '_' + str(batch_size) + '_' + str(early_stop) + '_out.txt', 'a+')
    ...
    for train_images, train_labels, valid_images, valid_labels, test_images, test_labels in zip(train_images, train_labels, valid_images, valid_labels, test_images, test_labels):
        ...
        with tf.Session() as session:
            ...
            saver = tf.train.import_meta_graph(save_dir + 'emodb_homework_2-0_C2:16,32,64,128;elu;l1;1.0;3;2|D:1500,800,400,200;relu;d;0.8_0.0001_52_6_4.meta')
            saver.restore(session,save_dir + 'emodb_homework_2-0_C2:16,32,64,128;elu;l1;1.0;3;2|D:1500,800,400,200;relu;d;0.8_0.0001_52_6_4.data-00000-of-00001')
            graph = session.graph
            ...
            dense_3 = tf.layers.dense(drop_2,100,activation=tf.nn.relu , name ="dense_3")
            drop_3 = tf.layers.dropout(dense_3,0.8,name ="drop_3")
            output2 = tf.layers.dense(drop_3,7,activation=tf.nn.relu , name ="output2")
            with tf.name_scope('optimizer') as scope:
                cross_entropy1 = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=output2)
            ...
            for epoch in range(FLAGS.epoch_num):
                ...
                for i in range(train_num_examples // batch_size):
                    ...
                    _, train_ce = session.run([train_op1, red_mean1], {x: batch_xs, y: batch_ys})
