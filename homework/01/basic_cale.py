...
def main(argv):
    ...
    ovrall_best_accuracy = float("inf")
    combos = [p for p in itr.product([100,200,300,400,500,600], repeat=2)]
    for c in combos:
        ...
        with tf.Session() as session:
            ...
            with open('/work/soh/charms/csce496/hw/hw01/modelParamsAndOut.txt', "a") as myfile:
                myfile.write("EPOCH: " + str(best_epoch) +
                "\nHIDDEN STRUCTURE: " + str(c) +
            ...
            if best_accuracy < ovrall_best_accuracy:
                    ovrall_best_validation_ce = best_validation_ce
                    ovrall_best_train_ce = best_train_ce
                    ovrall_best_conf_mx = best_conf_mx
                    ovrall_best_accuracy = avg_accuracy
                    ovrall_best_model = best_model
                    best_combo = c

        with open('/work/soh/charms/csce496/hw/01/combo_params)and_out.txt', "a") as myfile:
            myfile.write("BEST STRUCTURE: " + str(best_combo) +
            "\nBEST TRAIN LOSS: " + str(ovrall_best_train_ce) +
            "\nBEST VALIDATION LOSS: " + str(ovrall_best_validation_ce) +
            "\nBEST ACCURACY: " + str(ovrall_best_accuracy) +
            "\nBEST CONFUSION MATRIX: " + str(ovrall_best_conf_mx) +
            "\nBEST MODEL: " + str(ovrall_best_model))
