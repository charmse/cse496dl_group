...
def main(argv):
    ...
    with tf.name_scope('linear_model') as scope:
        hidden = tf.layers.dense(x, 300,kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=0.5),
                                         bias_regularizer=tf.contrib.layers.l2_regularizer(scale=0.5),
                                         activation=tf.nn.relu, name='hidden_layer1')
        hidden2 = tf.layers.dense(hidden, 500,kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=0.5),
                                         bias_regularizer=tf.contrib.layers.l2_regularizer(scale=0.5),
                                         activation=tf.nn.relu, name='hidden_layer2')
        hidden3 = tf.layers.dense(hidden2, 500,kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=0.5),
                                         bias_regularizer=tf.contrib.layers.l2_regularizer(scale=0.5),
