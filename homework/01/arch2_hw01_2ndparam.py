...
def main(argv):
    ...
    with tf.name_scope('linear_model') as scope:
        ...
        hidden3 = tf.layers.dense(hidden2, NEURONS_HIDDEN3,activation=tf.nn.relu, name='hidden_layer3')
        #dropped_hidden3 = tf.layers.dropout(hidden3, KEEP_PROB)
        hidden4 = tf.layers.dense(hidden3, NEURONS_HIDDEN4,activation=tf.nn.relu, name='hidden_layer4')
        #dropped_hidden4 = tf.layers.dropout(hidden4,KEEP_PROB)
        output = tf.layers.dense(hidden4, 10, name='output_layer')
